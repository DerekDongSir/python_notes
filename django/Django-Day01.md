...
```python
# 1.使用pycharm创建一个django的项目  （选择项目使用的虚拟环境）
# 2.先为某个模块创建一个app
# 3.在app的views.py文件定义一个函数 -- 逻辑代码
# 4.在视图的函数要返回响应 （a. HttpResponse("xxx")  b. render(request,"模板文件名")）
# 5.在urls.py文件中，为每一个视图函数提供一个访问的url
```
